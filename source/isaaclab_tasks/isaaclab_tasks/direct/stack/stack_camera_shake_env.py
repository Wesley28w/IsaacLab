...
from isaaclab.utils.math import sample_uniform, subtract_frame_transforms, quat_mul, quat_from_euler_xyz, euler_xyz_from_quat

# Non-privledged critic. Vanilla PPO
@configclass
class StackCameraShakeEnvCfg(DirectRLEnvCfg):
    episode_length_s = 20 # Will need to adjust
    decimation = 2
    action_space = 7 # (POS, ROT, Gripper)
    observation_space = 30 + 12 # (proprio + CNN embeddings)
    state_space = 30 + 35 + 12 # (proprio + gt poses + CNN embeddings) <-- asymmetric
    ...
    # camera (tiled)
    # TODO: # align with real camera and tune
    tiled_camera: TiledCameraCfg = TiledCameraCfg(
        prim_path="/World/envs/env_.*/Camera",
        offset=TiledCameraCfg.OffsetCfg(pos=(0.0, 0.0, 2.0), rot=(1.0, 1.0, 0.0, 0.0), convention="world"), # birds eye view looking straight down from center
        data_types=["rgb"],
        spawn=sim_utils.PinholeCameraCfg(
            focal_length=24.0, focus_distance=400.0, horizontal_aperture=20.995
        ),
        width=100,
        height=100,
    )
...
class StackCameraShakeEnv(DirectRLEnv):
    ...
    cfg: StackCameraShakeEnvCfg

    def __init__(self, cfg: StackCameraShakeEnvCfg, render_mode: str | None = None, **kwargs):
        super().__init__(cfg, render_mode, **kwargs)

        # setup the CNN - Use the log directory from the configuration
    ...
    def _setup_scene(self):
        ...
        # adding after the clone (NOTE: May be wrong and should do prior)
    ...
    def _compute_image_observations(self):
        poses = torch.cat([
            self.cube_one_pos, 
            euler_xyz_from_quat(self.cube_one_rot), 
            self.cube_two_pos, 
            euler_xyz_from_quat(self.cube_two_rot)
        ], dim=-1)

        poses_loss, embeddings = self.feature_extractor.step(
            self._tiled_camera.data.output["rgb"], poses
        )

        self.embeddings = embeddings.clone().detach()
        
        # log pose loss from CNN trainnig
        if hasattr(self, "extras") and "log" in self.extras:
            L = self.extras["log"]
            L["CNN/poses_loss"] = poses_loss
    ...
    def _compute_proprio_observations(self):
        ...
        obs = torch.cat(
            (
                ...
                self._scale(self.actions[:, :6], self.cfg.action_obs_scale), # (6)
                self._scale(self.actions[:, 6:], self.cfg.gripper_action_obs_scale) # (1)
            ), dim=-1
        )
        return obs  

    def _compute_gt_cube_poses(self):
        obs = torch.cat(
            (
                ...
                self._scale(self.ee_to_cube_one, self.cfg.pos_obs_scale), # (3)
                self._scale(self.ee_to_cube_two, self.cfg.pos_obs_scale), # (3)
                self._scale(self.cube_one_to_cube_two, self.cfg.pos_obs_scale), # (3) 
            ), dim=-1
        )
        return obs       

    def _get_observations(self) -> dict:
        proprio = self._compute_proprio_observations() # (30)
        gt_cube_poses = self._compute_gt_cube_poses() # (29)
        
        obs = torch.cat((proprio, self.embeddings), dim=-1)
        state = torch.cat((proprio, gt_cube_poses, self.embeddings), dim=-1) # embeddings is (12)
        
        return {"policy": torch.clamp(obs, -5.0, 5.0), "critic": torch.clamp(state, -5.0, 5.0)} # max input values of 5
